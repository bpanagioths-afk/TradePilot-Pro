...
def get_portfolio_overview(db: Session):
    total_accounts = db.query(MT5Account).count()

    active_accounts = db.query(MT5Account).filter(
    ...
    win_rate = _percentage(winning_trades, total_trades)
    ...
    winning_amounts = [
        value[0] for value in db.query(Trade.profit_money)
        .filter(Trade.profit_money > 0)
        .all()
    ...
    by_symbol = [
        {
            ...
            "value": _round(value),
            "percentage": _percentage(abs(value or 0), abs(total_profit))
        }
    ...
    by_direction = [
        {
            ...
            "percentage": _percentage(count or 0, total_trades)
        }
    ...
    return {
        ...
        "statistics": {
            ...
            "average_profit_per_trade": _round(average_profit_per_trade),
            "average_pips_per_trade": _round(average_pips_per_trade)
        },
        "allocation": {
            "by_symbol": by_symbol,
